...
![Simulation is a common tool for forecasting weather.](images/header-images/Day5pressureforecast.png)
...
1.  **Monte Carlo simulation:** A class of methods where values are repeatedly sampled/simulated from theoretical distributions that model a data generation process. Theoretical distributions, like the normal distribution, have closed-form representations[^closed-form] and a finite number of parameters like mean and variance.
...
[^closed-form]: A closed-form representation means the distribution can be described by an exact mathematical formula with a finite number of standard operations (addition, multiplication, exponentiation, etc.). For example, the normal distribution has a closed-form PDF, while some complex empirical distributions may not have simple mathematical expressions.
...
In practical terms, discrete random variables have outcomes you can list (even if infinite, like 1, 2, 3, ...).
:::
...
We show that a random variable is binomially distributed with
...
We can calculate the theoretical probability of a given draw from a binomial distribution using this PMF. For example, suppose we have a binomial distribution with $10$ trials and $p = \frac{1}{2}$. The probability of drawing exactly six $1$s and four $0$s is 

$$
p(X = 6) = \frac{10!}{6!4!} 0.5^6(1 - 0.5)^{10 - 6} = \frac{10 \times 9 \times 8 \times 7}{4 \times 3 \times 2 \times 1} \approx 0.2051
$$ {#eq-binomial-example}
...
-   calculate the probability (PMF) or density (PDF) for a specific value
...
The following answers the question: "What is the 10th percentile number of events in 10 trials when `p = 0.5`?"
...
The following randomly draws ten different binomially distributed random variables.
...
qbinom(pbinom(q = 2, size = 10, prob = 0.5), size = 10, prob = 0.5)
...
For the Poisson distribution, the parameter $\lambda$ is both the theoretical mean and theoretical variance. The PMF of a Poisson random variable is
...
3.  Visualize and compare both distributions.
...
We denote CDF with $F_X(x) = P(X \le x)$
:::
...
If we integrate the PDF from $-\infty$ to $x$ we get the CDF.
...
[Uniform random variables](https://en.wikipedia.org/wiki/Continuous_uniform_distribution) have equal probability for every value in the sample space. The distribution has two parameters: minimum and maximum. A standard uniform random variable has minimum = 0 and maximum = 1.

We show that a random variable is uniformly distributed with
...
The probability that an absolutely continuous random variable takes on any specific value is always zero because the sample space is uncountable. That is, $P(X = a) = 0$. Accordingly, we express the probability of observing events within a region for absolutely continuous random variables.

**This is crucial:** Unlike discrete random variables where $P(X = 3)$ has meaning, for continuous random variables, $P(X = 3.0000...)$ is always exactly 0. We can only calculate probabilities over intervals.
...
Observe how the normal distribution becomes a better approximation for Student's t-distribution when the degrees of freedom increase.

### Exponential Distribution

An [exponential random variable](https://en.wikipedia.org/wiki/Exponential_distribution) is the wait time between events for a Poisson process. It is useful for modeling wait time. For example, an exponential distribution can be used to model the wait time between arrivals in an emergency room between 1AM and 2AM. It has one parameter: rate ($\lambda$).
...
Suppose we have a sample of data $x_1, ..., x_n$. If the number of trials $n$ is already known, then $p$ is the only parameter for the binomial distribution that needs to be estimated. The MLE for $p$ is $\hat{p} = \frac{\sum_{i = 1}^n x_i}{mn}$. 
...
For a 2-dimensional multivariate normal distribution, the vector of means and variance-covariance matrix are:

$$
\vec\mu = \begin{bmatrix} \mu_1 \\ \mu_2 \end{bmatrix}, \quad
...
where $\sigma_1^2$ and $\sigma_2^2$ are the variances of the two variables, and $\rho$ is the correlation between them (or equivalently, the off-diagonal terms can be written as $Cov(X_1, X_2)$).
...
Functions for working with multivariate normal distributions are available from `library(MASS)`. @fig-mvnorm shows three different random samples from 2-dimensional multivariate normal distributions. 
...
**Monte Carlo but even easier this time:** Instead of solving complex math, we run the random process many times and count what happens.
...
2.  **Numerical integration** -- Integration, as noted above, is important for calculating probabilities and ultimately calculating quantities like expected value or confidence intervals. Monte Carlo methods can approximate multidimensional integrals that will never be directly solved by computers or simplify estimating probabilities when there are uncountably many potential outcomes (Solitaire).
...
Monte Carlo simulation basically repeats the ideas behind frequentist inferential statistics. If we can't measure every unit in a population, then we can sample a representative subset and estimate parameters about that population.
...
ggplot() +
  geom_polygon(data = circle_data, aes(x = x, y = y), fill = "blue", color = NA) +
...
Let $T_i$ be the time between arrivals and $T_i \sim exp(\lambda_1)$

Let $S_i$ be the service time and $S_i \sim exp(\lambda_2)$

From these two random variables, we can calculate the arrival times, departure times, and wait times for each customer.

-   The arrival times are the cumulative sum of the time between arrivals.
-   The wait times are zero if a person arrives after the person before them departs, and otherwise equal the difference between the prior person's departure and the current person's arrival.
...
#' @param t A value for the maximum time
#'
...
In other words, Monte Carlo simulation estimates assume the model is correct, which is almost certainly never fully true. Be transparent. Be humble.
