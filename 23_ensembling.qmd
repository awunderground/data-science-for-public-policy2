...
Participants use the 2012 Block-Group-Level Planning Database and were evaluated using mean squared error weighted by 2010 population.[^22_ensembling-1]

[^22_ensembling-1]: The Kaggle competition website says mean absolute error weighed by 2010 population.

244 teams and individuals competed. Bill Bame, a software developer from Maryland, won the competition with a mean squared error of 2.60. The top three finished use ensembled tree-based models like random forests and boosted trees [@erdman2014].
...
Ensembling combines predictions from multiple models into one prediction.
:::

Bagging, boosting, and stacking are three strategies for ensembling. We will cover bagging and boosting in this chapter. Random forests are an implementation of bagging. XGBoost is an implementation of boosting.
...
Bagging, short for bootstrap aggregation, is a general purpose ensembling method that reduces the variance of a predictive model by training the model on many bootstrap samples[^22_ensembling-2] and then ensembling the predictions.
...
[^22_ensembling-2]: This is the same bootstrap sampling used for nonparametric statistical inference.

Averaging a set of observations to reduce variance is a fundamental statistical idea. Recall the standard error of the sample mean. Suppose we have a set of data $x_1, x_2, ...x_n$. Let each observation be independent and have variance $\sigma^2$. The variance of the mean $\bar{x}$ is $\frac{\sigma^2}{n}$. Averaging the observations reduces variance. Averaging even more observations further reduces variance but at a diminishing rate.
...
Regularization is any technique that constrains model complexity to reduce overfitting and improve generalization. There are two main types:

1. **Loss function regularization** adds a penalty term to the loss function (e.g., LASSO and Ridge regression penalize large coefficients). This is explicit.
2. **Algorithmic regularization** constrains the model-fitting process itself without changing the loss function. This is implicit. 

Random forests use algorithmic regularization by randomly selecting predictors at each split, which prevents any single predictor from dominating and reduces correlation between trees.
:::
...
Random forests have a built-in method for estimating test error without needing a separate validation set or cross-validation.

::: callout-tip
## Out-of-Bag (OOB) Error

When training random forests, each tree is fit on a bootstrap sample of the data. On average, each bootstrap sample contains about 63% of the original observations, leaving 37% "out-of-bag."

For each observation, we can predict its value using only the trees that did not include it in their bootstrap sample. The OOB error is the average prediction error across all observations using their respective OOB predictions.
:::


This is a significant practical advantage of random forests over other ensemble methods.
...
Linear regression returns interpretable coefficients, and regression trees produce visualizable tree structures. Bagged trees and random forests return neither. 
...
Variable importance is an alternative approach that aims to explain which predictors contribute most to model predictions. For linear regression, variable importance is typically just the absolute value of the estimated coefficient. 

For regression with random forests, variable importance measures the total reduction in mean squared error from splits on each predictor across all trees. While valuable, variable importance has important limitations:

1. Correlated predictors
2. High-cardinality categoricals
3. Not causal

Permutation importance and SHAP values address some of these issues. 
