...
author: "Aaron R. Williams - Georgetown University"
...
One common approach is to look at representative units. For example, we could construct one observation that is representative and pass its values into a calculator. Then, we could extrapolate this experience to other observations.
...
1.  Create `new_tax_credit()`.
2.  Use `library(ggplot2)`, `expand_grid()`, and `map2_dbl()` to recreate the line for 3 children in @fig-benefit-curve.
:::
...
3.  Aggregate outcomes for the bottom 10 vintiles.
:::
...
For example, increasing Social Security benefits may crowd out retirement savings [@chetty2014].
:::
...
**Alignment** is the process of adjusting model coefficient or predicted values so output aligns with specified targets. These targets are aggregate outcomes like total income or state-level tax revenue.
:::
