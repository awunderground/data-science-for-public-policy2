...
![A Soviet Apartment that looks like a data frame.](images/header-images/Soviet_apartment.jpg)

Image by [lafleur](https://en.wikipedia.org/wiki/Brutalist_architecture#/media/File:Soviet_apartment.jpg)
...
This chapter introduces dplyr syntax using the March 2020 Annual Social and Economic Supplement (ASEC) to the [Current Population Survey](https://cps.ipums.org/cps/about.shtml) (CPS). 

The CPS is a monthly household survey and is the source of the official unemployment rate. The ASEC is a supplement to the CPS, sometimes called the "March Supplement", and is the official source for the poverty rate, household income, and the health insurance rate.
:::
...
We can use `glimpse(asec)` to quickly view the data. We can also use `View(asec)` to open up `asec` as a spreadsheet in RStudio. 
...
We're going to learn seven functions from `library(dplyr)` and one new piece of base R syntax that will be our main tools for manipulating tidy data frames. These functions and a few extensions outlined in the [Data Transformation Cheat Sheet](https://rstudio.com/resources/cheatsheets/) are the core of data analysis in the tidyverse. 
...
This works great until the goal is to select 99 of 100 variables. Fortunately, `-` can be used to list variables that should be dropped. You can also select all but multiple variables by listing them with the `-` symbol separated by commas. 
...
`mutate()` creates new variables or edits existing variables. We can use arithmetic arguments, such as `+`, `-`, `*`, `/`, and `^`. We can also use custom functions and functions from packages. For example, we can use `library(stringr)` for string manipulation and `library(lubridate)` for date manipulation. 
...
Conditional logic inside of `mutate()` with functions like `if_else()` and `case_when()` is key to working with data in R. 
...
`if_else()` is a useful function for conditionally creating or altering values in a variable. It has three main arguments:
...
R code can get repetitive when each operation needs to be assigned to a name with `<-`. The pipe, `|>`, allows lines of code to be chained together so the assignment operator only needs to be used once.
...
`summarize()` collapses many rows in a dataframe into fewer rows with summary statistics of the many rows. `n()`, `mean()`, and `sum()` are common summary statistics.
...
Renaming is useful with `summarize()` because variable names like `mean(ftotval)` and `mean(inctot)` are difficult to work with.
...
Mutating joins join one dataframe to another dataframe by matching values in variables common to both dataframes. The syntax is derived from *Structured Query Language* (SQL).
...
`left_join()` matches observations from the `y` dataframe to the `x` dataframe. It keeps all observations from the `x` (left) dataframe and adds columns from the `y` (right) dataframe wherever a match exists.
...
Zeta is in `reading_scores` but not in `math_scores`, so Zeta is dropped entirely because `left_join()` only keeps rows from `x`.
...
Reversing `x` and `y` changes the result. Now Zeta is kept because it's in `x` (`reading_scores`), but Zeta gets an `NA` for `math_score` because Zeta isn't in `y` (`math_scores`).

### `inner_join()`

`inner_join()` matches observations from the `y` dataframe to the `x` dataframe. It only keeps observations that have a match in both data frames.


```{r inner_join-example}
...
::: {.callout-tip}
[`library(tidylog)`](https://github.com/elbersb/tidylog) is a useful package for monitoring the behavior of joins. It prints out summaries of the number of rows in each dataframe that successfully join.
...
* `across()` can be used with `library(dplyr)` functions such as `summarize()` and `mutate()` to apply the same transformations to multiple columns. For example, it can be used to calculate the mean of many columns with `summarize()`. `across()` uses the same `tidyselect` select language and helpers discussed earlier to select the columns to transform.
