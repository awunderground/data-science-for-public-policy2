...
![](images/header-images/LeadPaint1.jpg)

Image by [Thester11](https://en.wikipedia.org/wiki/Lead_paint#/media/File:LeadPaint1.JPG)
...
We can use @eq-regression for all three motivations. 

- @eq-regression is a model of the conditional mean of $y$, which is a statistical summary.
- @eq-regression allows us to make inference about $\beta_j$ using $\hat\beta_j$.
- @eq-regression allows us to make predictions, $\hat{y_i}$, given $\vec{x_i}$.
