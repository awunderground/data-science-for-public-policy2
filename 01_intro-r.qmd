...
`NA` is a missing value of length 1 in R. `NA`s are powerful representations in R with special properties. `NA` is a contagious value in R that will override all calculations [@wickham_r_2017, Section 20.2].
...
**Style note:** Objects should be given names that are "concise and meaningful" [@style, Chapter 2: Syntax]. Generally the names should be nouns and only use lowercase letters, numbers, and underscores `_` (this is referred to as snake case).
...
All R programming will involve some base R, but much base R has been replaced with new tools that are more concise. Just know that at some point you may end up on a Stack Overflow page that looks like alphabet soup because it's in a paradigm that you have not learned. Chapter 27 of *R for Data Science (2e)* (sometimes abreviated R4DS) provides a great introduction to base R [@wickham_r_2023]. 
