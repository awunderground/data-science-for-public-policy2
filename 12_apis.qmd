...
The Census API requires a user-specific API key for all API calls. ([You can sign up here](https://api.census.gov/data/key_signup.html)) This API key is simply passed as part of the path in the HTTP request.
...
# make the URL with API key
url <- str_glue(
  "https://api.census.gov/data/2014/pep/natstprc?get=STNAME,POP&DATE_=7&for=state:*&key={census_key}"
)
...
map_df(fips, .f = get_acs, census_key = census_key)
