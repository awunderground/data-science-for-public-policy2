...
#| code-fold: true
#| fig-cap: "Simulated data with polynomial regression model predictions"
...
#| code-fold: true
#| warning: false
#| fig-cap: "Simulated data with a piecewise-constant regression model predictions"
...
$$
y_i = \beta_0 + \beta_1b_1(x_i) + \beta_2b_2(x_i) + \cdot\cdot\cdot + \beta_kb_K(x_i) + \epsilon_i
$$
...
#| code-fold: true
#| warning: false
#| fig-cap: "Simulated data with predictions using different regression splines"
...
Multivariate Adaptive Regression Splines (MARS) are an alternative to GAMs for capturing non-linear patterns in data using linear regression and splines.
...
Visualization is a useful way to unpack model predictions. There are many different approaches to visualizing model predictions, depending on whether you want local or global interpretability.

::: callout-tip
## Ceteris Paribus Plot

A visualization of predicted values varying one predictor for an observation while holding all other predictors constant. 
...
A ceteris paribus plot for many different observations at the same time.
:::

::: callout-tip
## Additive Component Plot

A visualization of the functional form $f(x_j)$ for one predictor, holding all other predictors at a reference value (typically 0). Valid for additive models only — for additive models, the choice of reference value changes the y-intercept but not the shape of the relationship.
:::

::: callout-tip
## Partial Dependence Plot

A visualization of the marginal effect of a predictor. This is equivalent to averaging the ICE curves. 

:::

The first two plots focus on local interpretability because they focus on predictions for individual observations. The last two plots focus on global interpretability because they try to describe average model behavior.

In the following case study we will show ICE plots with PDP plots on top and additive component plots.
...
#' Create an ICE Plot with a PDP Plot on Top
#'
#' @param wf A trained workflow object
#' @param training_data The training data for the trained workflow object
#' @param n The number of observations to show in the ICE plot
#' @return A ggplot object with ICE curves (gray) and PDP (red)
#'
ice_and_pdp <- function(wf, training_data, n = 50) {
  ...
  explainer <- explain_tidymodels(
    ...
    data = training_data |> 
      select(-non_return_rate),
    y = training_data$non_return_rate
  )
  
  # predict the profiles for randomly sampled observations in the training data
  ice_data <- predict_profile(
    explainer,
    new_observation = slice_sample(training_data, n = n),
    variables = "Renter_Occp_HU_ACS_13_17"
  )
  
  # predict profiles for every observation for the PDP plot
  # this is needed because of some issues inside of DALEXtra
  fitted_model <- wf |> 
    fit(training_data)
  
  x_seq <- seq(0, 7400, length.out = 40)
  
  pdp_df <- map(x_seq, \(x_val) {
    training_data |>
      mutate(Renter_Occp_HU_ACS_13_17 = x_val) |>
      predict(fitted_model, new_data = _) |>
      pull(.pred) |>
      mean()
  }) |>
    list_c() |>
    tibble(x = x_seq, mean_pred = _)
  
  # visualize!
  ggplot() +
    geom_line(
      data = as.data.frame(ice_data),
      aes(x = Renter_Occp_HU_ACS_13_17, y = `_yhat_`, group = `_ids_`),
    ...
    labs(
      title = "ICE Plot with Partial Dependence",
      subtitle = "Individual profiles (gray) + PDP over all training data (red)",
      x = "Renter-Occupied Housing Units",
      y = "Predicted Non-Return Rate"
    )
...
```{r}
ice_and_pdp(wf = lm1_wf, training_data = pdb_train)

```

#### Additive Component Plot

The following code creates an additive component plot for `Renter_Occp_HU_ACS_13_17`. Because these models are additive, the value of other predictors only shifts the line vertically and doesn't change the shape of the relationship.
...
bind_cols(
  ...
  geom_line() +
  labs(title = "Additive Component Plot for Linear Regression")
...
```{r}
ice_and_pdp(wf = lm2_wf, training_data = pdb_train)
...
bind_cols(
  ...
  ggplot(aes(Renter_Occp_HU_ACS_13_17, .pred)) +
  geom_line() +
  labs(title = "Additive Component Plot for Linear Regression with Transformations")
...
```{r}
ice_and_pdp(wf = enet_tuned_wf, training_data = pdb_train)
...
bind_cols(
  ...
  ggplot(aes(Renter_Occp_HU_ACS_13_17, .pred)) +
  geom_line() +
  labs(title = "Additive Component Plot for Elastic Net")
...
```{r}
ice_and_pdp(wf = polynomial_wf, training_data = pdb_train)
...
bind_cols(
  ...
  ggplot(aes(Renter_Occp_HU_ACS_13_17, .pred)) +
  geom_line() +
  labs(title = "Additive Component Plot for Polynomial Regression")
...
```{r}
ice_and_pdp(wf = poly_enet_tuned_wf, training_data = pdb_train)

```

#### Additive Component Plot

This model inclues `step_interact()`, so we skip the additive component plot.
...
```{r}
ice_and_pdp(wf = gam_final_wf, training_data = pdb_train)
...
bind_cols(
  ...
  ggplot(aes(Renter_Occp_HU_ACS_13_17, .pred)) +
  geom_line() +
  labs(title = "Additive Component Plot for GAM/Regression Splines")
...
```{r}
ice_and_pdp(wf = mars_wf, training_data = pdb_train)
...
bind_cols(
  ...
  ggplot(aes(Renter_Occp_HU_ACS_13_17, .pred)) +
  geom_line() +
  labs(title = "Additive Component Plot for MARS")
