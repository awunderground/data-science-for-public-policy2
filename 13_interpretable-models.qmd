...
Interactions extend polynomials to consider multiplying predictors by other predictors in addition to multiplying them by themselves. Including pairwise or triplewise interactions complicates interpretation but can improve the fit of the model to the data. To read more about how to interpret interaction term regression coefficients, see [section 11.2](https://book.stat420.org/categorical-predictors-and-interactions.html) of an online textbook for a University of Illinois regression class.
...
Simple pairwise interactions for the small planning database example explodes the number of predictors from 23 to 276.[^pairwise]

[^pairwise]: We now have two types of predictors: the initial 23 predictors and all 2-way interactions between them. To calculate all of the unique combinations, we can use the [combination formula](https://en.wikipedia.org/wiki/Combination). Applying this to our case, we have ${23 \choose 2} = \frac{23!}{(23 - 2)!2!} = 253$. 23 original predictors and 253 interactions together makes 276 total predictors.
...
Step functions break the range of a predictor into bins and then fit different constant (intercept only models) in each bin. @fig-piecewise-constant-regression shows piecewise constant regression fit to the simulated example 1 data. The example uses 9 separate regressions. We can also say that this example has 8 knots, or the number of break points between regions.
