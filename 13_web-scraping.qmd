...
We explored pulling data from web APIs in [@sec-api-intro]. With web APIs, stewards are often carefully thinking about how to share information. This will not be the case with web scraping. 
...
## Introduction and Motivation {#sec-web-scraping-intro}
...
-   We worked with the text of executive orders going back to the Clinton Administration when learning text analysis. Unfortunately, the Federal Register doesn't publish a massive file of executive orders. So we iterated through websites for each executive order, scraped the text, and cleaned the data.
...
[^04_web-scraping-1]: We are not lawyers. This is not official legal advice. If in doubt, please contact a legal professional.
...
[R for Data Science (2e)](https://r4ds.hadley.nz/webscraping#scraping-ethics-and-legalities) contains a clear and approachable rundown of legal considerations for gathering information from the web. We adopt their three-part standard of "public, non-personal, and factual", which relate to terms of service, personally identifiable information, and copyright.
...
Organizations that host websites [are increasingly burdened by web crawlers](https://www.eff.org/deeplinks/2025/06/keeping-web-under-weight-ai-crawlers) used to gather up training data for training generative AI models. Many of these crawlers ignore robots.txt. In response, Cloudfare [has introduced technology](https://www.cloudflare.com/press/press-releases/2025/cloudflare-just-changed-how-ai-crawlers-scrape-the-internet-at-large/) that give organizations more control over how their information is collected. This remains a developing area for technology, litigation, and norms.
...
The rise of generative AI makes urgent the consideration of giving credit. These models consume massive amounts of training data, and it isn't clear where the training data come from or the legal and ethical implications of the training data.[^04_web-scraping-3]
...
1. Start [here](https://www.countyhealthrankings.org/health-data).
2. Using the interface at the bottom of the page, we can navigate to the page for "Virginia."
3. Next, we can click "Download Virginia data sets."

That's a lot of clicks to get [here](https://www.countyhealthrankings.org/health-data/virginia/data-and-resources).
...
It's simple to build a website with Quarto because it abstracts away most of web development. For example, Markdown is just a shortcut to write HTML. For example, clicking "Render" generates CSS and JavaScript to support final .html documents. Web scraping requires us to learn more about web development than rendering Quarto documents.

The user interface of websites can be built with just HTML, but most websites contain HTML, CSS, and JavaScript. The development of the interfaces of websites with HTML, CSS, and JavaScript is called front-end web development. 

::: {.callout-tip}
## Hyper Text Markup Language

**Hyper Text Markup Language (HTML)** is the standard language for creating web content. HTML is a markup language, which means it has code for creating structure and formatting. 
:::
...
**Cascading Style Sheets (CSS)** describes how HTML elements should be styled when they are displayed. 
:::
...
<h2 class='important'>Big Title!</h2>
<p>My first paragraph.</p>
<p id='special-paragraph'>My second paragraph.</p>
</body>
</html>
```
...
CSS selectors can also reference HTML IDs. For example, the following selector changes the style of the one element with `id='special-paragraph'`
...
[HTML tables](https://www.w3schools.com/html/html_tables.asp) store information in tables in websites using the `<table>`, `<tr>`, `<th>`, and `<td>` tags. If the data of interest are stored in tables, then it can be trivial to scrape the information. 
