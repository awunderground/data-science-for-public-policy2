...
We'll typically use default bandwidths. Think of bandwidths like binwidths for the relative frequency histograms we discussed in @sec-binwdith. Using smaller bandwidths leads to noisier KDEs but more precision. Increasing the bandwidth leads to smoother KDEs but less precision.
...
[^algebra1]: Recall your algebra class where, to move the vertex of a parabola in the x-axis some distance $h$, you updated the formula $y = x^2$ to $y = (x-h)^2$. The change to the Gaussian function is analogous. 
