...
Mutating and filtering joins are essential for combining multiple tables during data analysis.
...
For example, we may have a data frame with student identifiers and math grades, and a data frame with student identifiers and science grades. We can use a mutating join to create a data frame with math and science grades.
:::
...
Let there be two data frames `x` and `y` and let both data frames have a key variable that uniquely identifies rows. In practice, in R, we often use the following functions: 
...
Import is the process of loading data into R from files (e.g., .csv or .xlsx), databases, web application programming interfaces ([Chapter @sec-api-intro]), or web pages with web scraping ([Chapter @sec-web-scraping-intro]). 
...
[^1]: The instinct to include these comments is good. The execution is poor because it creates big headaches for people using programming languages. We suggest using a data dictionary instead.
...
Tidying is the process of reorganizing your data to be in a tidy format. The defining opinion of the tidyverse is its wholehearted adoption of tidy data. [Tidy data has three features](https://r4ds.hadley.nz/data-tidy#fig-tidy-structure):
...
`pivot_longer()` reorients data so that key-value pairs expressed as column name-column value are column value-column value in adjacent columns. `pivot_longer()` is commonly used for tidying data and for making data longer for `library(ggplot2)`. `pivot_longer()` has three essential arguments:
...
Let's walk through a series of examples that show the process of tidying untidy data.
...
Transforming data includes filtering to observations of interest, creating new variables for analysis, and summarizing data. There are many ways to transform data. We will focus on a few common transformations. 
...
`str_detect()` is useful for **detecting matches** in strings, which can be useful with `filter()`. 

Consider the executive orders data set and suppose we want to return executive orders that contain the word `"Virginia"`. 
...
`str_sub()` can **subset strings** based on positions within the string. 

Consider an example where we want to extract state FIPS codes from county FIPS codes. 
...
`str_pad()` is useful for **managing lengths**. 

Consider the common situation when zeros are dropped from the beginning of FIPS codes.
...
`str_replace()`, `str_replace_all()`, `str_remove()`, and `str_remove_all()` can delete or **modify** parts of strings. 

Consider an example where we have course names and we want to delete everything except numeric digits.[^2]

[^2]: This example uses regular expressions (regex). Visit [R4DS (2e)](https://r4ds.hadley.nz/regexps) for a review of regex.
...
`str_c()` and `str_glue()` are useful for **joining** strings. 

Consider an example where we want to "fill in the blank" with a variable in a data frame.

```{r}
tibble(fruit = c("apple", "banana", "cantaloupe")) |>
...
1. Use `mutate()` and `library(stringr)` to create a variable for `percentile` from the earlier SOI exercise. For instance, `"p99"` should be `"99"`.
2. Use `as.numeric()` to convert the string from step 1 into a numeric value.
3. Create a data visualization with `percentile` on the x-axis. 
...
The [forcats cheat sheet](https://forcats.tidyverse.org/) is a useful resource for working with factors.
...
`library(lubridate)` contains functions for extracting components from dates like the year, month, day, and weekday. Consider the following data set about [full moons](https://www.timeanddate.com/moon/phases/usa/washington-dc) in Washington, DC in 2023. 
...
3. Create a vector with the dates of your first 120 birth days. *Do you use periods or durations?* 
...
4. The tidyverse also contains useful packages for handling strings (`stringr`), factors (`forcats`), and dates (`lubridate`). 
