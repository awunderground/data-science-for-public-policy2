...
*R for Data Science (2e)* [displays](https://r4ds.hadley.nz/intro.html#fig-ds-diagram) the first steps of the data science process as "Import", "Tidy", and "Transform". Recall from the previous lecture techniques for importing data like `read_csv()`, for tidying data like `pivot_longer()`, and for transforming data like `mutate()`.
...
* `inner_join(x, y)` appends variables from `y` onto `x` but only keeps observations for which there is a match between in the `x` and `y`data frames.
