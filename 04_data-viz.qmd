...
![Charles Minard's famous data visualization of Napoleon's Russian Campisng of 1812.](images/header-images/Minard.png)
...
Data visualization is a general-purpose tool with varied uses.
...
`library(ggplot2)` has numerous advantages over point-and-click tools like Microsoft Excel.
...
`ggplot2` is an R package for data visualization that was developed during Hadley Wickham's graduate studies at Iowa State University. 

"gg" stands for the grammar of graphics. The grammar of graphics is a theoretical framework that breaks all data visualizations into their component pieces, which was developed by Leland Wilkinson [@wilkinson1999grammar].

`ggplot2` is formalized in *A Layered Grammar of Graphics* [@layered-grammar]. With the *layered* grammar of graphics, Wickham extends Wilkinson's grammar of graphics and implements it in R. The cohesion is impressive, and the theory flows to the code which informs the data visualization process in a way not reflected in other data viz tools. Learning the theory helps with learning the code and learning the code helps with learning the theory. 

Good data management is foundational to working with `ggplot2`. 

* Data frames, including tibbles, are the only appropriate input for `ggplot2`. 
* The toughest part of data visualization is often data munging--the process of cleaning and transforming messy data into the necessary format. We'll sometimes need to use `dplyr` to change our data to change our visualizations.
...
1. Create a new folder and project.
2. Create a new `.R` script in your directory called `04_data-visualization.R`. 
...
This example demonstrates *data*, *aesthetic mappings*, and *geometric objects*.
...
:::{.callout-tip}
**Aesthetic mappings** are directions for how data are mapped in a plot in a way that humans can perceive. Aesthetic mappings include linking variables to the x-position, y-position, color, fill, shape, transparency, and size. 
:::
...
`facet_wrap` creates small multiples, in order, until the variable is exhausted. It does not provide much macro structure to the visualization. `facet_grid` creates a macro structure where each panel represents the combination of one level on the macro x-axis with one level on the macro y-axis.[^facet-geo] 
