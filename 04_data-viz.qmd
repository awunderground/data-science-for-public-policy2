...
ggplot2 is an R package for data visualization that was developed during Hadley Wickham's graduate studies at Iowa State University. ggplot2 is formalized in *A Layered Grammar of Graphics* by Hadley Wickham @layered-grammar.
...
`facet_wrap` and `facet_grid` are similar but have distinct functions. As the diagram below illustrates, `facet_wrap` organizes a 1 dimensional set of charts, but wraps the charts in two dimensions to preserve space. `facet_grid` organizes a set of charts in two dimensions. [^facet-geo] 
...
recent_storms %>%
  ...
  geom_polygon(data = states, aes(long, lat, group = group),
               fill = "grey", color = "#ffffff", linewidth = 0.05) +
