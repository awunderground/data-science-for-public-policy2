...
Monte Carlo methods and resampling methods have a wide range of applications. Monte Carlo simulation is used by election forecasters to predict electoral outcomes and econometricians to understand the properties of estimators. Resampling methods are used in machine learning and causal inference. Both ar fundamental methods for agent-based models including microsimulation.
...
A [binomial random variable](https://en.wikipedia.org/wiki/Binomial_distribution) is the number of ones observed in $n$ repeated Bernoulli trials.
...
@fig-binomial shows 1,000 random draws from a binomial distribution with 10 trials and `p = 0.5`. The theoretical distribution is overlayed in red.
...
This PMF is visualized in @fig-die. We can sample from this PDF with
...
-   [**Geometric RV:**](https://en.wikipedia.org/wiki/Geometric_distribution) Number of Bernoulli trials up to and including the $1^{st}$ success
-   [**Negative Binomial RV:**](https://en.wikipedia.org/wiki/Negative_binomial_distribution) Number of Bernoulli trials up to and including the $r^{th}$ success
...
2.  Identify the finite number of parameters associated with the probability distribtuion.
3.  Calculate the parameters that maximize the probability of the observed data.

::: {.callout-tip}
## Likelihood

Let $\vec{x}$ be observed data and $\theta$ be a parameter or parameters from a chosen probability distribution. The **likelihood** is the probability of the observed data conditional on values of the parameters.
...
Suppose we have three independent normally-dstributed random variables. 
...
There are at least three popular resmpling methods:
