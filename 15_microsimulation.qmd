...
![Microsimulation can model complex systems by modeling the actions of many individual actors](images/header-images/ApisDorsataHive.jpg)

Image by [Bksimonb](https://en.wikipedia.org/wiki/Honey_bee#/media/File:ApisDorsataHive.jpg)
...
1.  Create a new, well-documented function called `alternative_tax_credit()` based on `new_tax_credit()` that models the following proposed tax credit:
    * Ignore the first $20,000 of family income.
    * Create a maximum benefit of \$4,000 for one child, \$4,500 for two children, and \$5,000 for three children.
    * Reduce the benefit by \$0.05 for every dollar of income in excess of $20,000.
...
proposal2 <- asec |>
  mutate(
    ...
    benefit_counterfactual = map2_dbl(
      .x = nchild, 
      .y = ftotval, 
      .f = newer_tax_credit
    )
...
proposal2 |>
  mutate(benefit_change = benefit_counterfactual - benefit_baseline) |>
    mutate(ftotval_decile = ntile(ftotval, n = 10)) |>
  group_by(ftotval_decile) |>
  summarize(
    min_income = min(ftotval),
...
Here are a few examples of transitions that could be modeled:

-   If an individual will acquire more education.
-   If an individual will work.
-   If an individual will marry, divorce, or become widowed.
...
[^3]: The standard error of the mean is proportional to $\frac{1}{\sqrt{n}}$, so it decreases as the sample size increases.
